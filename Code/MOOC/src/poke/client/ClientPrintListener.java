...
public class ClientPrintListener implements CommListener {
	...
	public void onMessage(eye.Comm.Request msg) {
		...
		logger.info("inside on message generic");
		logger.info("in client print on mesage " );
		if (msg.getHeader().getRoutingId().getNumber() == Header.Routing.PING_VALUE)
			...
			if(msg.getBody().hasReqList())
			{
				ClientUtil.printCourseList(msg); 
			}
			if(msg.getBody().hasSignUp())
			{
				ClientUtil.printSuccess(msg.getHeader().getReplyMsg());
			}
			
			/*logger.info("inside on message of jobs");   ClientUtil.printSuccess(msg.getHeader().getReplyMsg()); */
