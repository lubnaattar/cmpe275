 ...
 * 
 * 
 * the heartbeatMgr will keep the connection between the two nodes active.
...
public class HeartbeatManager extends Thread {
	...
	public ConcurrentHashMap<Channel, HeartbeatData> getOutgoingQueue()
	{
	...
	//		System.out.println("incomming Heart beat status is"+hd.getStatus());
		}
	}
	...
	public void addOutgoingChannel(String nodeId, String host, int mgmtport, Channel ch, SocketAddress sa) {
		if (!outgoingHB.containsKey(ch)) {
			...
			//System.out.println("Inside heart manager after setting connection to send heart beat.");
	...
	protected void addAdjacentNode(HeartbeatData node) {
		...
		if (!incomingHB.containsKey(node.getNodeId()))
		{
			...
			System.out.println("node"+node.getNodeId()+"is set in incomingHB list in Heartbeat Managerclass");
			//System.out.println("after setting node id and node in incommingHB");
		}
	}
	...
	private Management generateHB() {
		...
		b.setBeat(h.build());
		System.out.println("Message generated in Heartbeat Manager"+b.getBeat());
		return b.build();
	}
	@Override
	public void run() {
			...
			try {
				...
				if (outgoingHB.size() > 0) {
					...
					for (HeartbeatData hd : outgoingHB.values()) {
						...
						//	System.out.println("Heart beat status is before setting the beat staus"+hd.getStatus());
							hd.setStatus(BeatStatus.Active);
						//	System.out.println("Beat Status is Active"+hd.getStatus());
